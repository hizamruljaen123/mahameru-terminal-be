...
def calculate_all(df, include=None):
    ...
    def should_calc(key):
        ...
        if 'summary' in include:
            # Core indicators needed for signals and dashboards
            core_indicators = ['sma', 'ema', 'bb', 'rsi', 'macd', 'stoch', 'adx', 'atr', 'cci', 'willr', 'sar', 'ichimoku', 'vwap', 'volume']
            if key in core_indicators: return True
...
def generate_signals(df, indicators):
    ...
    sa = safe(ichi.get('senkou_a', [0])[-1])
    sb = safe(ichi.get('senkou_b', [0])[-1])
    tk = safe(ichi.get('tenkan', [0])[-1])
    kj = safe(ichi.get('kijun', [0])[-1])
    ...
    # CCI & Williams %R
    cci = safe(indicators.get('cci', [0])[-1]) if isinstance(indicators.get('cci'), list) else 0
    willr = safe(indicators.get('willr', [0])[-1]) if isinstance(indicators.get('willr'), list) else 0
...
def analyze(symbol):
    ...
    try:
        ...
        # --- FIX: Clean data to prevent NULL indicators ---
        # Indonesian stocks often have gaps or NaNs which break calculations
        df = df.ffill().dropna()
        if len(df) < 50:
            return jsonify({
                "error": f"Insufficient data for {symbol} ({len(df)} days). "
                         "Calculations require at least 50 valid trading days."
            }), 400
        ...
        # Fix: 'summary' should also trigger signal generation
        is_summary = not include_list or 'all' in include_list or 'summary' in include_list or 'adx' in include_list
        score = generate_signals(df, indicators) if is_summary else {"verdict": "DATA_LIMITED"}
        ...
        # Condensed history for AI analysis (Match with analysis period)
        history_df = df.copy()
        history_data = {
            "dates": history_df.index.strftime('%Y-%m-%d').tolist(),
            "close": [round(float(x), 2) for x in history_df['Close'].tolist()],
            "open":  [round(float(x), 2) for x in history_df['Open'].tolist()],
            "high":  [round(float(x), 2) for x in history_df['High'].tolist()],
            "low":   [round(float(x), 2) for x in history_df['Low'].tolist()],
            "volume": [int(x) for x in history_df['Volume'].tolist()]
        }
