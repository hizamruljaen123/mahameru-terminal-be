...
def get_detail_cache(symbol, period):
...
def set_detail_cache(symbol, period, data):
    key = f"{symbol}_{period}"
    DETAIL_CACHE[key] = {'timestamp': time.time(), 'data': data}
...
def get_coin_detail(symbol: str, period: str = "1mo"):
    ...
    cached = get_detail_cache(symbol, period)
    ...
    set_detail_cache(symbol, period, result)
...
def get_historical_data(symbol: str, period: str = "1mo", interval: str = "1d"):
    ...
    try:
        ticker = yf.Ticker(f"{symbol}-USD")
        hist = ticker.history(period=period_map.get(period, "1mo"), interval=interval)
        if hist.empty:
...
def get_technical_indicators(symbol: str, period: str = "1mo"):
    ...
    try:
        ...
        return {"status": "success", "data": indicators[-90:]}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
