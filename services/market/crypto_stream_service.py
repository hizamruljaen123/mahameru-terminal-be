...
async def binance_stream_manager():
    ...
    for s in SYMBOLS:
        sl = s.lower()
        symbol_streams.append(f"{sl}@kline_1m/{sl}@kline_5m/{sl}@kline_15m/{sl}@kline_1h/{sl}@trade/{sl}@bookTicker/{sl}@markPrice/{sl}@depth5")
            ...
            async with websockets.connect(url, max_size=5 * 1024 * 1024) as ws:
                while True:
                    ...
                    if "@bookTicker" in stream:
                        ...
                        depth_full = {
                            "b": [[float(p), float(q)] for p, q in payload["bids"]],
                            "a": [[float(p), float(q)] for p, q in payload["asks"]]
                        }
                        market_cache[symbol]["depth_full"] = depth_full
                        processed_data = {"type": "depth_full", "symbol": symbol, "data": depth_full}
    ...
    snapshot = {
        ...
        "data": {
            s: {
                ...
                "depth_full": market_cache[s].get("depth_full", {"b": [], "a": []})
            } for s in SYMBOLS
        }
    }
    try:
