...
class MacroAnalyzer:
    ...
    def get_full_macro(self):
        return {"etf_flows": self.get_etf_flows(),
            ...
            "macro_correlation": self.get_macro_correlation(),
            "fed_policy": self.get_fed_policy(),
            "exchange_reserves": self.get_exchange_reserves()}

    def get_macro_correlation(self, period="90d"):
        """Compute correlation with traditional markets"""
        ck = f"macro_corr_{period}"
        c = self._cached(ck)
        if c: return c
        try:
            # Download traditional market data
            data = yf.download(["BTC-USD", "^GSPC", "^TNX", "GC=F", "^VIX"], period=period, progress=False)['Close']
            if data.empty: return {"error": "No data"}
            
            # Normalize to last price
            last = data.iloc[-1]
            normalized = (data / last).dropna()
            
            # Create time series for chart
            series = {}
            for col in normalized.columns:
                symbol = col.replace("-USD", "").replace("^", "")
                series[symbol] = [{"date": i.strftime('%Y-%m-%d'), "value": round(float(v), 4)} 
                                 for i, v in normalized[col].items()]
            
            return self._store(ck, series)
        except Exception as e: return {"error": str(e)}

    def get_fed_policy(self):
        """Simulate Fed policy tracker"""
        ck = "fed_policy"
        c = self._cached(ck)
        if c: return c
        try:
            # Simulated Fed policy history (in real app, fetch from FRED API)
            policies = [
                {"date": "2026-05-01", "rate": 5.25, "action": "Hold", "outlook": "Neutral"},
                {"date": "2026-03-19", "rate": 5.25, "action": "Hold", "outlook": "Neutral"},
                {"date": "2025-12-17", "rate": 5.50, "action": "Hike", "outlook": " hawkish"},
                {"date": "2025-10-29", "rate": 5.50, "action": "Hold", "outlook": "Neutral"},
                {"date": "2025-09-17", "rate": 5.00, "action": "Cut", "outlook": "Dovish"},
                {"date": "2025-07-30", "rate": 5.00, "action": "Hold", "outlook": "Neutral"},
                {"date": "2025-06-11", "rate": 5.25, "action": "Hike", "outlook": "Hawkish"},
                {"date": "2025-05-01", "rate": 5.25, "action": "Hold", "outlook": "Neutral"},
                {"date": "2025-03-19", "rate": 5.50, "action": "Hike", "outlook": "Hawkish"},
                {"date": "2025-01-29", "rate": 5.25, "action": "Hold", "outlook": "Neutral"},
            ]
    ...
    def get_exchange_reserves(self):
        """Simulate exchange reserves monitor"""
        ck = "exchange_reserves"
        c = self._cached(ck)
        if c: return c
        try:
            # Simulated exchange reserve data
            reserves = [
                {"exchange": "BINANCE", "reserves": 325000000000, "change_7d": -0.02, "trend": "OUTFLOW"},
                {"exchange": "COINBASE", "reserves": 185000000000, "change_7d": 0.01, "trend": "INFLOW"},
                {"exchange": "KRAKEN", "reserves": 45000000000, "change_7d": -0.03, "trend": "OUTFLOW"},
                {"exchange": "BYBIT", "reserves": 38000000000, "change_7d": 0.02, "trend": "INFLOW"},
                {"exchange": "HUOBI", "reserves": 28000000000, "change_7d": -0.01, "trend": "OUTFLOW"},
            ]
            
            return self._store(ck, {"reserves": reserves})
        except Exception as e: return {"error": str(e)}
