...
class QuantAnalyzer:
    ...
    def get_full_quant(self, symbol):
        return {"correlation": self.compute_correlation_matrix(symbol),
            ...
            "risk_metrics": self.compute_risk_metrics(symbol),
            "stress_testing": self.compute_stress_testing(symbol)}

    def compute_risk_metrics(self, symbol, period="1y"):
        """Compute VaR, CVaR, and tail risk metrics"""
        ck = f"risk_{symbol}_{period}"
        c = self._cached(ck)
        if c: return c
        try:
            df = yf.Ticker(f"{symbol}-USD").history(period=period)
            ...
            worst_5 = returns[returns <= np.percentile(returns, 5)]
            worst_1 = returns[returns <= np.percentile(returns, 1)]
            cvar_95 = abs(worst_5.mean()) if len(worst_5) > 0 else var_95
            cvar_99 = abs(worst_1.mean()) if len(worst_1) > 0 else var_99
            
            # Tail risk index (combined measure)
            tail_risk_30d = min(100, max(0, (var_95 * 100 + cvar_95 * 50) / 2))
            ...
            return self._store(ck, {
                ...
                "tail_risk_7d": round(float(tail_risk_30d * 0.5), 2),
                "tail_risk_30d": round(float(tail_risk_30d), 2),
                "tail_risk_90d": round(float(tail_risk_30d * 0.8), 2),
                "tail_risk_180d": round(float(tail_risk_30d * 0.6), 2)
            })
        except Exception as e: return {"error": str(e)}

    def compute_stress_testing(self, symbol, period="1y"):
        """Compute stress test scenarios"""
        ck = f"stress_{symbol}_{period}"
        c = self._cached(ck)
        if c: return c
        try:
            df = yf.Ticker(f"{symbol}-USD").history(period=period)
            ...
            worst_day = df['ret'].min()
            best_day = df['ret'].max()
            vol_90d = df['ret'].rolling(90).std().mean()
            
            scenarios = [
                {"name": "2020 CRASH", "loss": round(float(abs(worst_day) * 100 * 2), 2), 
                 "probability": 0.5, "impact": "HIGH"},
                {"name": "FLASH CRASH", "loss": round(float(abs(worst_day) * 100 * 3), 2),
                 "probability": 0.3, "impact": "CRITICAL"},
                {"name": "LIQUIDATION", "loss": round(float(vol_90d * 100 * 2), 2),
                 "probability": 1.0, "impact": "MODERATE"},
                {"name": "BLACK SWAN", "loss": round(float(vol_90d * 100 * 4), 2),
                 "probability": 0.1, "impact": "CRITICAL"}
            ]
            
            return self._store(ck, {"scenarios": scenarios})
        except Exception as e: return {"error": str(e)}
