...
# ─────────────────────────────────────────────────────────────────────────────
# PORTFOLIO OPTIMIZATION ENGINE (PyPortfolioOpt)
# ─────────────────────────────────────────────────────────────────────────────
...
async def optimize_portfolio(req: OptimizeRequest):
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        period = req.window if req.window in ['1mo', '3mo', '6mo', '1y', '5y', 'max'] else '1y'
        
        def _fetch_adj_close(symbol: str) -> pd.Series:
            hist = yf.Ticker(symbol).history(period=period)
            if "Close" in hist:
                return hist["Close"].rename(symbol)
            return pd.Series(dtype=float, name=symbol)

        futures = [loop.run_in_executor(_MARKET_EXEC, _fetch_adj_close, s) for s in symbols]
        series_list = await asyncio.gather(*futures, return_exceptions=True)
        ...
        if len(price_dict) < 2:
            return {"status": "error", "detail": "Insufficient historical data for optimization"}

        df = pd.DataFrame(price_dict).dropna(how="all").ffill().bfill()
        ...
        weights_dict = {sym: float(w) for sym, w in cleaned_weights.items()}
        ...
        return {
            ...
            "metrics": {
                "expected_annual_return_pct": round(float(exp_ret) * 100, 2),
    ...
    try:
        ...
        def _fetch():
            ...
            snapshot = {
                ...
                "pegRatio": _safe(info.get("pegRatio")),
                ...
                "trailingAnnualDividendYield": _pct(info.get("trailingAnnualDividendYield")),
                ...
                "totalCashPerShare": _safe(info.get("totalCashPerShare")),
                "totalDebt": _safe(info.get("totalDebt")),
                "freeCashflow": _safe(info.get("freeCashflow")),
                "leveredFreeCashflow": _safe(info.get("leveredFreeCashflow")),
                ...
                "quarterlyRevenueGrowth": _pct(info.get("quarterlyRevenueGrowth")),
                ...
                "grossProfits": _safe(info.get("grossProfits")),
                "netIncomeToCommon": _safe(info.get("netIncomeToCommon")),
