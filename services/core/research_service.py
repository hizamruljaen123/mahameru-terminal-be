...
from copilot.models import get_available_models_list, get_model_config, MODEL_PROVIDERS
...
# Model configuration is now handled by copilot.models library
...
def get_models():
    """Return all registered models from the centralized library."""
...
@app.route('/api/research/start', methods=['GET'])
@app.route('/research/api/research/start', methods=['GET'])
def research_start():
    """
    Initial handshake for the Deep Research Pipeline.
    Returns metadata about the research request.
    """
    symbols = request.args.get('symbols', '').strip().upper()
    analysis_type = request.args.get('analysis_type', 'full')
    
    return jsonify({
        "status": "success",
        "message": f"Deep Research Pipeline ready for {symbols}",
        "symbols": symbols,
        "analysis_type": analysis_type,
        "stages": 7,
        "note": "This is an AI-powered 7-stage research pipeline. Results will be streamed.",
        "data": {
            "symbol": symbols.split(',')[0] if symbols else "BBRI.JK",
            "type": analysis_type
        }
    })
...
def analyze_report():
    ...
    def generate():
        # Resolve model configuration via centralized library
        ...
        if not api_key:
            yield f"data: {json.dumps({'error': f'API Key for provider {provider} not found.'})}\n\n"
            return

        if provider == MODEL_PROVIDERS["GOOGLE"]:
            # Gemini API Logic
            contents = []
        ...
        # OpenAI-compatible API Logic (DeepSeek, DIT, Moonshot, etc.)
        try:
...
def analyze_compare():
    ...
    Accepts multi-symbol data and streams AI analysis for one stage.
    """
    data = request.json or {}
