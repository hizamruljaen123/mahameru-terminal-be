...
def _build_rich_response(
    ...
    for result in tool_results:
        ...
        if not result.get("success"):
            # Suppress "connection failed" errors for secondary tools to reduce noise
            error_msg = str(result.get('error', 'Unknown error'))
            if "connection attempts failed" in error_msg.lower():
                logger.warning(f"Connection failed for {tool_name}, skipping component.")
                continue
                
            components.append({
                "type": "markdown",
                "data": f"⚠️ **{tool_name}**: {error_msg}",
            })
            continue
...
def _transform_vessel_radar(tool: str, data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Transform radar geocoding data into a vessel_radar component."""
    return [{
        "type": "vessel_radar",
...
# ---------------------------------------------------------------------------
# Deep Research transformer (Triggers SSE Stream in UI)
# ---------------------------------------------------------------------------


def _transform_research(tool: str, data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Transform research start metadata into an SSE stream trigger component."""
    symbols = data.get("symbols", data.get("symbol", "BBRI.JK"))
    analysis_type = data.get("analysis_type", "full")
    
    return [
        {
            "type": "markdown",
            "data": f"### 🔬 Deep Research Pipeline: **{symbols}**\n{data.get('note', '')}"
        },
        {
            "type": "sse_stream",
            "symbols": symbols,
            "analysis_type": analysis_type,
            "title": f"Start 7-Stage Analysis for {symbols}"
        }
    ]
...
_COMPONENT_TRANSFORMERS: Dict[str, Callable] = {
    ...
    "get_vessel_radar": _transform_vessel_radar,
    "run_deep_research": _transform_research,
}
