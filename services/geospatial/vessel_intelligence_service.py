...
execute_transaction = db_utils.execute_transaction
import logging
...
if not API_TOKEN:
    logger.warning("VESSEL_INTEL_API_TOKEN environment variable not set. Some features may be limited.")
    API_TOKEN = "DEBUG_TOKEN" # Fallback for local development
...
@app.get("/intelligence/radar")
async def get_radar_coordinates(
    location: str = Query(..., min_length=2),
):
    """
    Resolves a location name (e.g. 'Singapore Strait') to coordinates 
    for the VesselFinder radar widget.
    """
    try:
        geo = geocode_place(location)
        if not geo:
            # Fallback to general search if specific geocode fails
        ...
        return {
            ...
            "lat": geo["lat"],
            "lon": geo["lon"],
            "zoom": 10, # Default zoom for radar
            "timestamp": datetime.utcnow().isoformat() + "Z"
        }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Radar geocoding error: {str(e)}")
