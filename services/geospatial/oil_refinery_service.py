...
def get_refineries(
    ...
    limit: int = Query(100, description="Limit records", ge=1, le=1000),
    offset: int = Query(0, description="Offset records", ge=0)
):
    try:
        ...
        query = """
            SELECT id, nama_kilang, negara, provinsi, kota,
                   latitude, longitude, kapasitas, tahun_operasi, status
            FROM oil_refineries WHERE 1=1
        """
        params = []
        ...
        # Get total count
        count_query = "SELECT COUNT(*) as total FROM oil_refineries WHERE 1=1"
        count_params = []
        if q:
            count_query += " AND nama_kilang LIKE %s"
            count_params.append(f"%{q}%")
        if country:
            count_query += " AND negara LIKE %s"
            count_params.append(f"%{country}%")
        cursor.execute(count_query, tuple(count_params))
        total = cursor.fetchone()['total']

        # Add pagination
        query += " ORDER BY nama_kilang ASC LIMIT %s OFFSET %s"
