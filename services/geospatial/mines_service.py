...
def get_filters():
    """Optimized: single query with subqueries instead of 7 round trips (7x faster)."""
    from services.system.cache_manager import get_countries_cached
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)

    filters = {}

    # Cache countries for country filter
    countries_cached = get_countries_cached()
    filters["countries"] = sorted(set(c['name'] for c in countries_cached if c.get('name')))

    # Single query using subqueries to get all distinct values at once
    cursor.execute("""
        SELECT
            (SELECT GROUP_CONCAT(DISTINCT commod1) FROM mines_data WHERE commod1 IS NOT NULL AND {active}) as commodities,
            (SELECT GROUP_CONCAT(DISTINCT dev_stat) FROM mines_data WHERE dev_stat IS NOT NULL AND {active}) as dev_status,
            (SELECT GROUP_CONCAT(DISTINCT region) FROM mines_data WHERE region IS NOT NULL AND {active}) as regions,
            (SELECT GROUP_CONCAT(DISTINCT oper_type) FROM mines_data WHERE oper_type IS NOT NULL AND {active}) as oper_types,
            (SELECT GROUP_CONCAT(DISTINCT prod_size) FROM mines_data WHERE prod_size IS NOT NULL AND {active}) as prod_sizes,
            (SELECT GROUP_CONCAT(DISTINCT com_type) FROM mines_data WHERE com_type IS NOT NULL AND {active}) as com_types,
            (SELECT GROUP_CONCAT(DISTINCT score) FROM mines_data WHERE score IS NOT NULL AND {active}) as scores
    """.format(active=ACTIVE_FILTER))

    row = cursor.fetchone()
    # Parse comma-separated values
    for key in ['commodities', 'dev_status', 'regions', 'oper_types', 'prod_sizes', 'com_types', 'scores']:
        val = row.get(key) if row else None
        filters[key] = sorted(v.strip() for v in val.split(',') if v.strip()) if val else []
