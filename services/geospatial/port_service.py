...
def get_continents():
    try:
        ...
        continents = sorted(set(c['continent'] for c in countries if c.get('continent')))
...
def get_countries():
    ...
    try:
        ...
        # Sort by name
        countries = sorted(countries, key=lambda c: c.get('name', c.get('code', '')))
        result = [{"code": c['code'], "name": c['name'] or c['code']} for c in countries]
