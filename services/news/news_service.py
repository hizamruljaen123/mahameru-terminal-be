...
# Resolve absolute paths to ensure reliability regardless of how the script is called
current_file = os.path.abspath(__file__)
current_dir = os.path.dirname(current_file)
# news_service.py is in services/news/ -> go up two levels to reach mahameru-terminal-be
project_root = os.path.dirname(os.path.dirname(current_dir))

# Add project root to path for 'utils' and other root-level packages
if project_root not in sys.path:
    sys.path.insert(0, project_root)

# Add services/system to path for 'db' and 'cache_manager'
system_dir = os.path.join(project_root, 'services', 'system')
...
from services.system import cache_manager
...
def build_source_list(assigned_categories=None):
    ...
    Uses cached feedsource data (5 min TTL) for performance.
    Priority order:
    ...
    # --- Step 1: Load ALL active sources from cached feedsource ---
    try:
        # Use cache_manager's cached feedsource (5-min TTL)
        from services.system.cache_manager import get_feedsource_cached
        db_rows = get_feedsource_cached(active_only=True)

        if assigned_categories:
            # Filter by assigned categories (case-insensitive)
            assigned_upper = [c.upper() for c in assigned_categories]
            db_rows = [row for row in db_rows if (row.get('category') or 'UNCATEGORIZED').upper() in assigned_upper]
        ...
        for row in db_rows:
            ...
            sources.append({
                ...
                'trust': 8 if not is_google else 5,
        ...
        log.info(f"DB_SOURCES: Loaded {len(sources)} active sources from feedsource cache")
...
def detect_sector_ai(company_name, ticker_symbol):
    """Fallback to AI to detect company sector and category if yfinance fails."""
    api_key = os.getenv('GEMINI_API_KEY')
    if not api_key:
        return ""
    
    log.info(f"AI_SECTOR_DETECT: Identifying sector for '{company_name}' ({ticker_symbol})...")
    prompt = f"""
    Identify the primary industrial sector for this company. 
    Use broad standard sectors (e.g., Financial, Energy, Technology, Consumer Defensive, Healthcare, Basic Materials, Industrials).
    
    Company: {company_name}
    Ticker: {ticker_symbol}
    
    Response MUST be a single word representing the sector.
    """
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            sector = text.split('\n')[0].split('.')[0].strip()
            log.info(f"AI_SECTOR_DETECT: Resolved to '{sector}'")
            return sector
    except Exception as e:
        log.warning(f"AI_SECTOR_DETECT_FAILED: {e}")
...
def search_news():
    ...
    # --- FIX: Ticker normalization and name enrichment for better search results ---
    # Headlines often use just the ticker (BBCA) or company name (Bank Central Asia)
    # but rarely the full ticker with suffix (BBCA.JK).
    
    # 1. Base queries
    search_queries = [query_term]
    
    # If the query is JUST a ticker (e.g. BBCA.JK or BBCA), try to get the full name
    if len(query_term.split()) == 1:
        try:
            norm_q = query_term.upper()
            if not norm_q.endswith(".JK") and len(norm_q) == 4:
                # Likely Indonesian ticker without suffix
                norm_q += ".JK"
            
            log.info(f"SEARCH_ENRICH: Resolving company name for ticker '{norm_q}'...")
            ticker = yf.Ticker(norm_q)
            info = ticker.info
            if info:
                full_name = info.get('longName') or info.get('shortName')
                if full_name:
                    log.info(f"SEARCH_ENRICH: Resolved '{norm_q}' to '{full_name}'")
                    # TIGHT FORMAT: [Ticker] [Full Name] PERUSAHAAN Berita
                    tight_query = f"{norm_q} {full_name} PERUSAHAAN Berita"
                    search_queries.insert(0, tight_query) # Prioritize the tight query
                    search_queries.append(full_name)
                    # Add simple name if full name is too long
                    simple_name = full_name.split(' Tbk')[0].split(' (Persero)')[0]
                    if simple_name != full_name:
                        search_queries.append(simple_name)
        except Exception as ye:
            log.warning(f"SEARCH_ENRICH_FAILED: {ye}")

    # 2. Ticker variants
    if ".JK" in query_term.upper():
        ticker_only = query_term.upper().split()[0].replace(".JK", "")
        search_queries.append(ticker_only)
    
    # 3. Individual keywords (if it's a long query)
    keywords = [k for k in query_term.split() if len(k) > 2]
    if len(keywords) > 1:
    ...
    try:
        ...
        # Build multi-term OR condition
        like_clauses = []
        params = []
        for q in search_queries:
            like_clauses.append("title LIKE %s OR description LIKE %s")
        ...
        sql = f"""
            ...
            FROM article WHERE {where_clause}
        ...
        # --- Step 3: Live Google News fallback if no results found ---
        if not results and query_term:
            # Use the resolved name for search if available, otherwise original query
            resolved_name = next((q for q in search_queries if len(q.split()) > 1), query_term)
            log.info(f"SEARCH_FALLBACK: No local results. Attempting live Google News fetch for '{resolved_name}'...")
            try:
                # Use Indonesian locale for .JK stocks, English otherwise
                hl = "id" if ".JK" in query_term.upper() else "en"
                gl = "ID" if ".JK" in query_term.upper() else "US"
                
                query_encoded = urllib.parse.quote(resolved_name)
                gnews_url = f"https://news.google.com/rss/search?q={query_encoded}&hl={hl}-{gl}&gl={gl}&ceid={gl}:{hl}"
                
                feed = feedparser.parse(gnews_url)
                for entry in feed.entries[:20]:  # Limit to top 20 fresh hits
                    # Basic mapping to match our schema
                    pub_date_struct = getattr(entry, 'published_parsed', None)
                    timestamp = time.mktime(pub_date_struct) if pub_date_struct else time.time()
                    
                    import re
                    clean_desc = re.sub(r'<[^>]*>', '', getattr(entry, 'summary', ''))
                    
                    item = {
                        "id": f"live_{hashlib.md5(entry.link.encode()).hexdigest()[:8]}",
                        "title": entry.title,
                        "link": entry.link,
                ...
                # Perform sentiment analysis on live results if any
                if results and any(r.get('is_live') for r in results):
                    live_items = [r for r in results if r.get('is_live')]
                    log.info(f"SEARCH_SENTIMENT: Analyzing sentiment for {len(live_items)} live articles...")
                    try:
                        sentiment_url = os.getenv('SENTIMENT_SERVICE_URL', 'http://127.0.0.1:5008/api/sentiment/analyze')
                        s_resp = requests.post(sentiment_url, json=live_items, timeout=10.0)
                        if s_resp.status_code == 200:
                            analyzed_data = s_resp.json()
                            # Map back to results
                            for i, r in enumerate(results):
                                if r.get('is_live'):
                                    # Find matching link in analyzed_data
                                    match = next((a for a in analyzed_data if a['link'] == r['link']), None)
                                    if match:
                                        r['sentiment'] = match.get('sentiment', 'NEUTRAL')
                                        r['sentimentScore'] = match.get('sentimentScore', 0.5)
                                        r['impactScore'] = match.get('impactScore', 0.5)
                        else:
                            log.warning(f"SEARCH_SENTIMENT_FAILED: HTTP {s_resp.status_code}")
                    except Exception as se:
                        log.error(f"SEARCH_SENTIMENT_ERROR: {se}")

            except Exception as fe:
                log.error(f"SEARCH_FALLBACK_FAILED: {fe}")

        # --- Step 4: Trash News Filtering (Noise Reduction) ---
        # Filter results to ensure they mention the ticker, company name, or sector
        log.info(f"SEARCH_FILTER: Validating {len(results)} articles for relevance...")
        
        # Get sector if available for broader filtering
        sector_term = ""
        if 'ticker' in locals() and ticker:
            try:
                sector_term = ticker.info.get('sector', '')
                # AI Fallback if yfinance has no sector info
                if not sector_term:
                    sector_term = detect_sector_ai(full_name or query_term, query_term)
            except: pass

        filtered_results = []
        # Keywords to match against
        match_keywords = [q.lower() for q in search_queries if len(q) > 2]
        if sector_term: match_keywords.append(sector_term.lower())
        
        # Localized sector terms for Indonesian stocks
        if ".JK" in query_term.upper():
            if "Financial" in sector_term: match_keywords.extend(["perbankan", "finansial", "bank"])
            elif "Energy" in sector_term or "Basic Materials" in sector_term: match_keywords.extend(["tambang", "batubara", "minerba", "energi"])
            elif "Consumer" in sector_term: match_keywords.extend(["konsumsi", "retail"])
            elif "Technology" in sector_term: match_keywords.extend(["teknologi", "digital"])
            elif "Healthcare" in sector_term: match_keywords.extend(["kesehatan", "farmasi"])
        
        for r in results:
            text = (r.get('title', '') + " " + r.get('description', '')).lower()
            # Must contain at least one significant keyword (Ticker, Name, or Sector)
            if any(kw in text for kw in match_keywords):
                filtered_results.append(r)
            else:
                log.debug(f"SEARCH_FILTER: Removed trash news: {r.get('title')[:50]}...")
        
        results = filtered_results
        log.info(f"SEARCH_FILTER: Retained {len(results)} relevant articles.")

        results.sort(key=lambda x: x.get('timestamp', 0), reverse=True)
        
        # Final clean-up of descriptions for all results (remove HTML tags like <img>)
        import re
        for r in results:
            if r.get('description'):
                r['description'] = re.sub(r'<[^>]*>', '', r['description']).strip()
...
def get_country_intel_news():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        # Use FULLTEXT index instead of LIKE for 10-40x faster search
